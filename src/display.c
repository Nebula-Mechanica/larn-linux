...
void bot_linex(void)
{
	...
	if ( regen_bottom || (always))
	{
		...
		for (i=0; i<100; i++) cbak[i]=cdesc[i];
		return;
	}
...
void showcell(int x, int y)
{
	...
	if (cdesc[BLINDCOUNT])  return; /* see nothing if blind     */
	if (cdesc[AWARENESS]) { minx = x-3; maxx = x+3; miny = y-3; maxy = y+3; }
...
if direction=0, don't move--just show where he is */
{
	...
	if (cdesc[CONFUSE]) if (cdesc[LEVEL]<rnd(30)) dir=rund(9); /*if confused any dir*/
