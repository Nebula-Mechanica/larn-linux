...
void fntchange(int how)
{
	...
	case 1: 
		...
		fch(how,&cdesc[STRENGTH]);     break;
	case 2:
		lprcat("Your intelligence");    
		fch(how,&cdesc[INTELLIGENCE]);     break;
	case 3:
		lprcat("Your wisdom");          
		fch(how,&cdesc[WISDOM]);     break;
	case 4: 
		lprcat("Your constitution");    
		fch(how,&cdesc[CONSTITUTION]);     break;
	case 5:
		lprcat("Your dexterity");
		fch(how,&cdesc[DEXTERITY]);     break;
	case 6:
		lprcat("Your charm");
		fch(how,&cdesc[CHARISMA]);     break;
	case 7: 
		j=rnd(level+1);
		if (how < 0)
		{
		...
		{ lprintf("You gain %d hit point",(int)j);  if (j>1) lprcat("s!"); else lprc('!'); raisemhp((int)j); }
		bottomline();       break;

	case 8: j=rnd(level+1);
		if (how > 0)
		{
			lprintf("You just gained %d spell",(int)j);  raisemspells((int)j);
			if (j>1) lprcat("s!"); else lprc('!');
		}
		else
		{
			lprintf("You just lost %d spell",(int)j);  losemspells((int)j);
			if (j>1) lprcat("s!"); else lprc('!');
		}
		bottomline();       break;
	...
	case 9: j = 5*rnd((level+1)*(level+1));
		if (how < 0)
		{
			...
			if (j>1) lprcat("s!"); else lprc('!'); loseexperience(j);
		}
		else
		{
...
static void move_cursor(int *xx, int *yy, int cdir )
{
