...
int diag(void)
{
	...
	lprintf("\n\nPlayer attributes:\n\nHit points: %2d(%2d)",(int)cdesc[HP],(int)cdesc[HPMAX]);
	lprintf("\ngold: %d  Experience: %d  Character level: %d  Level in caverns: %d",
		(int)cdesc[GOLD],(int)cdesc[EXPERIENCE],(int)cdesc[LEVEL],(int)level);
	...
	{
		lprintf("%19s  %2d  %3d ",monster[i].name,(int)monster[i].level,(int)monster[i].armorclass);
		lprintf(" %3d  %3d  %3d  ",(int)monster[i].damage,(int)monster[i].attack,(int)monster[i].defense);
		lprintf("%6d  %3d   %6d\n",(int)monster[i].gold,(int)monster[i].hitpoints,(int)monster[i].experience);
	}
	...
	for (i=0; i<=MAXMONST+8; i++)
	{
		...
		lprintf("\n%20s   %2d/%2d/%2d       %2d/%2d/%2d       %2d/%2d/%2d",
			monster[i].name,
			(int)(hit/2),(int)max(0,dam+2),(int)(monster[i].hitpoints/(dam+2)+1),
			(int)((hit+2)/2),(int)max(0,dam+10),(int)(monster[i].hitpoints/(dam+10)+1),
			(int)((hit+5)/2),(int)max(0,dam+20),(int)(monster[i].hitpoints/(dam+20)+1));
	}
	...
	for (j=0; j<100; j+=10)
	{
		lprintf("\nc[%2d] = ",(int)j); for (i=0; i<9; i++) lprintf("%5d ",(int)cdesc[i+j]);
	}
	...
	for (i=0; i<16; i++)  { lprintf("  %5d",(int)rndcount[i]); if (i==7) lprc('\n'); }
...
int savegame(char *fname)
{
	...
	for (k=0; k<MAXLEVEL+MAXVLEVEL; k++) {
		if (beenhere[k])
			lwrite( (char*)&cell[(int) k*MAXX*MAXY], sizeof(struct cel)*MAXY*MAXX );
	}
...
void restoregame(char *fname)
{
	...
	for (k=0; k<MAXLEVEL+MAXVLEVEL; k++) {
		...
		lrfill( (char*)&cell[(int) k*MAXX*MAXY],	sizeof(struct cel)*MAXY*MAXX );
	}
...
#ifdef ANTICHEAT
	if (_unlink(fname) < 0) fcheat(); 
	for (k=0; k<6; k++) if (cdesc[k]>99) greedy();
	if (cdesc[HPMAX]>999 || cdesc[SPELLMAX]>125) greedy();
#endif
